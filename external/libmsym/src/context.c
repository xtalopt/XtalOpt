...
msym_error_t msymGetBasisFunctions(msym_context ctx, int *length, msym_basis_function_t **basis){
    ...
    if(length) *length = 0;
    if(basis) *basis = NULL;
    msymSetErrorDetails("Basis functions are not included in XtalOpt's vendored libmsym subset");
...
msym_error_t msymGetCharacterTable(msym_context ctx, const msym_character_table_t **ct){
    (void) ctx;
    if(ct) *ct = NULL;
    msymSetErrorDetails("Character tables are not included in XtalOpt's vendored libmsym subset");
