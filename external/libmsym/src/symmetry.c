        ...
        for(int i = 0;i < sopsl;i++){
            ...
            if(sops[i].type == IMPROPER_ROTATION){
                ...
                if(sops[i].order != 2){
                    ...
                    // Use the local copy 'v' of cinf[0]->v: the realloc() of
                    //   rsops below can move the buffer that cinf[0] points
                    //   into, so dereferencing cinf[0] here is a use-after-free.
                    add = vparallel(sops[i].v,v,thresholds->angle) || (vperpendicular(sops[i].v,v,thresholds->angle) && inv[0]);
                }
