...
void tabPrintTransform(int r, int c, const double *M,int indent);
...
void tabPrintTransform(int r, int c, const double *M,int indent) {
    ...
    for(int i = 0;i < r;i++){
        for(int j = 0;j<c;j++){
            char *pre = signbit(M[i*c + j]) ? "" : " ";
            char *post1 = "\t";
            char *post2 = (j == (c - 1)) ? (i == (r - 1)) ? "" : ";" : " ";
            
            printf("%s%.8lf%s%s",pre,M[i*c + j],post1,post2);
        }
        printf("%s",(i == (r - 1)) ? "]\n" : "\n ");
        tabprintf(" ", indent);
    }
...
void printTransform(int r, int c, const double *M) {
    printf("\n[");
    for(int i = 0;i < r;i++){
        for(int j = 0;j<c;j++){
            char *pre = signbit(M[i*c + j]) ? "" : " ";
...
void printSubspace(msym_character_table_t *ct, int l, msym_subrepresentation_space_t *srs){
    for(int k = 0;k < l;k++){
        ...
        for(int i = 0;i < srs[k].salcl;i++){
            ...
            tabPrintTransform(srs[k].salc[i].d,srs[k].salc[i].fl,(const double *) space,1);
        }
    }
}
