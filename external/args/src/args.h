...
// Controls parse-error behavior. Defaults to true:
// - true: print parse errors to stderr and exit non-zero.
// - false: ap_parse() returns false on parse errors.
...
// - Returns false if parsing failed due to parse errors (when
//   ap_set_exit_on_error(..., false) is set) or memory allocation failures.
...
// Returns true if the last ap_parse() on the parser failed due to invalid
// command-line arguments.
bool ap_had_parse_error(ArgParser* parser);

// Returns the parse error string from the last failed ap_parse(), or NULL.
