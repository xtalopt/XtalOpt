...
static ArgParser* g_parse_error_parser = NULL;
static jmp_buf* g_parse_error_jmp = NULL;

static void ap_set_parse_error(ArgParser* parser, const char* format_string, va_list args);
...
static void ap_clear_parse_error(ArgParser* parser) {
    ArgParser* root = parser->root_parser;
    root->had_parse_error = false;
    free(root->parse_error);
    root->parse_error = NULL;
}


static void ap_set_parse_error(ArgParser* parser, const char* format_string, va_list args) {
    ArgParser* root = parser->root_parser;
    root->had_parse_error = true;
    free(root->parse_error);
    root->parse_error = NULL;

    va_list args_copy;
    va_copy(args_copy, args);
    int len = vsnprintf(NULL, 0, format_string, args_copy);
    va_end(args_copy);
    if (len < 0) {
        ap_set_memory_error_flag(root);
        return;
    }
    ...
    vsnprintf(message, (size_t)len + 1, format_string, args);
    root->parse_error = message;
}
...
static void exit_with_error(const char* format_string, ...) {
    ...
    if (g_parse_error_parser && !g_parse_error_parser->exit_on_error) {
        ap_set_parse_error(g_parse_error_parser, format_string, args);
        va_end(args);
        if (g_parse_error_jmp) {
    ...
    va_end(args);
    fprintf(stderr, " (--help,-h for options)");
    fprintf(stderr, "\n");
    exit(1);
}
...
void ap_set_exit_on_error(ArgParser* parser, bool exit_on_error) {
    parser->root_parser->exit_on_error = exit_on_error;
}
...
bool ap_parse(ArgParser* parser, int argc, char** argv) {
    ...
    ap_clear_parse_error(parser);

    if (argc == 0) {
        return true;
    }

    parser->root_parser->zeroth_root_arg = argv[0];
    ...
    ArgParser* previous_parse_error_parser = g_parse_error_parser;
    jmp_buf* previous_parse_error_jmp = g_parse_error_jmp;
    g_parse_error_parser = parser->root_parser;

    bool parse_error = false;
    jmp_buf parse_error_jmp;
    g_parse_error_jmp = &parse_error_jmp;
    ...
    g_parse_error_parser = previous_parse_error_parser;
    g_parse_error_jmp = previous_parse_error_jmp;
    ...
    return !parser->had_memory_error && !parser->root_parser->had_parse_error;
}
...
bool ap_had_parse_error(ArgParser* parser) {
    return parser->root_parser->had_parse_error;
}


const char* ap_get_parse_error(ArgParser* parser) {
    return parser->root_parser->parse_error;
}
