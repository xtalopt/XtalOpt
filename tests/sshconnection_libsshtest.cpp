/**********************************************************************
  SSHConnection - Unit test for the SSHConnection class: libssh
...
private:
  SSHConnectionLibSSH* conn = nullptr;
...
{
  Common::message("Ignore the following warning about a failed connection "
...
void SSHConnectionLibSSHTest::executeLargeOutput()
{
  ...
  // chroot jails.
  QString command = QString("i=0; while [ \"$i\" -le %1 ]; do echo 000; "
                            "i=$((i + 1)); done").arg(LIBSSH_BUFFER_SIZE);
