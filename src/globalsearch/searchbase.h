...
static QString run_log_filename = "output.log";
...
/**
 * @class SearchBase searchbase.h <globalsearch/searchbase.h>
 *
 ...
 *
 * SearchBase is the main class in libglobalsearch. It contains the
...
public:
   ...
   */
  explicit SearchBase(AbstractDialog* parent = nullptr);
  ...
  */
   ...
   * structure, and return the selected xtal's index in the list.
   *
   * The probability of a crystal being selected may be obtained from either
   *   multi-objective scalar function or Pareto optimization.
   *
   * @param structures The list of structures to consider
   * @param parentsPoolSize The size of the parents pool. After probabilities are
   *                generated, the low probability structures will be trimmed
   *                off until parentsPoolSize is reached.
  ...
  /**
   * Legacy "get probabillity list" function: kept for test suit and benchmarking.
   * @param structures
   * @param poolSize
   * @return Basic scalar generalized fitness values for structures
   */
  QList<QPair<Structure*, double>> getProbabilityList(const QList<Structure*>& structures, size_t poolSize);
  ...
  /**
   * @brief getReferenceEnergiesVector
   * @return The vector of atom counts and energies for reference energies,
   * to be added to hull input data.
   */
  virtual std::vector<double> getReferenceEnergiesVector() = 0;

  /**
   * @brief getChemicalSystem
   * @return The alphabetically sorted symbols for all elements in the search
   * from XtalOpt class
   */
  virtual QList<QString> getChemicalSystem() const = 0;

  /**
   * Called after each successful optimization to update
   * distance above hull and Pareto front information for all structures.
   * @sa QueueManager
   */
  void updateHullAndFrontInfo();

  /**
   * Saves snapshots of the hull in the 'movie' directory in local working
   * directory with every successfull hull calculation.
   */
  bool saveHullSnapshot();
  ...
  void setObjectivesTyp(ObjType f) {m_objectives_typ.push_back(f);};
  ...
  int     getObjectivesNum()      const {return m_objectives_typ.size();};
  ObjType getObjectivesTyp(int i) const {return m_objectives_typ.at(i);};
  ...
  void resetObjectives() {m_objectives_typ.clear(); m_objectives_exe.clear();
                          m_objectives_out.clear(); m_objectives_wgt.clear();};
  /// Remove the "i"th objective
  void removeObjective(int i) {m_objectives_typ.removeAt(i); m_objectives_exe.removeAt(i);
                               m_objectives_out.removeAt(i); m_objectives_wgt.removeAt(i);};

  /// These are variables initiated from the input for use in optimization.

  /// Print some extra infor in the output
  bool m_verbose;

  /// Print hull snapshots to movie folder
  bool m_saveHullSnapshots;

  /// Number of decimal places for objective values passed to prob cal func
  ///     (= -1) number as is; (>=0) number of decimal places; default is 6
  ...
  /// Type of optimization: Basic (scalar fitness) or Pareto
  QString m_optimizationType;

  /// Use tournament selection in Pareto optimization?
  bool m_tournamentSelection;

  /// Restrict the pool size in tournament selection?
  bool m_restrictedPool;

  /// Use crowding distance in Pareto optimization?
  ...
  /// How many structures to produce before halting search
  int maxNumStructures;
...
protected:
  ...
  QList<ObjType> m_objectives_typ; // type of optimization: minimization/maximization/filtration
