...
public:
  ...
  */
   ...
   *
   * For enthalpy, the probability is calculated by:
   *
   ...
   * factor.
   *
   * If hardness is used instead, the equation will be:
   *
   * p_i = N * (1 - (H_max - H_i) / (H_max - H_min) )
   *
...
@endcode
   ...
   * @param useHardness Whether or not to use hardness instead for the
   *                    probability. If true, the structures need to be
   *                    sorted by hardness.
   *
   * @sa Structure::sortByEnthalpy()
   * @sa Structure::sortByVickersHardness()
   *
