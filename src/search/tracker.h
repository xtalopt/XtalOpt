...
/**
 * @class Tracker tracker.h <search/tracker.h>
 * @brief The Tracker contains a list of unique Structures.
 ...
 * Most functions do not lock themselves, so the caller must hold
 * the proper lock while using them.
 */
...
public:
   ...
   * @note The caller must hold the lock while using the list.
   */
   ...
   * @note Does not lock; the caller must hold at least a read lock.
   */
   ...
   * @note Does not lock; the caller must hold the write lock.
   */
   ...
   * @note Takes the write lock itself; do not call this while
   * already holding the lock for read.
   */
  void deleteAllStructures();

  /** Delete a Structure in its QObject thread when possible. */
  static void deleteStructure(Structure* s);
...
signals:
   ...
   */
  void newStructureAdded(Search::Structure* s);
