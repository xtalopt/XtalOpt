...
/**
 * The plot widget used by the plot tab; wraps the Qwt plot with
 * point picking that selects the matching structure.
 */
...
public:
  ...
  const std::vector<std::unique_ptr<QwtPlotMarker>>& plotMarkers() const
  {
    return m_markerList;
  };
  ...
  QwtPlotMarker* selectedMarker() const { return m_selectedMarker; };
