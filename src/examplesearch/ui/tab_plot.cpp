...
namespace ExampleSearch {
  ...
  TabPlot::TabPlot( ExampleSearchDialog *parent, ExampleSearch *p ) :
  ...
  void TabPlot::plotTrends()
  {
    ...
    m_plotObject = new PlotObject (Qt::red,
                                   PlotObject::Points,
                                   4,
                                   PlotObject::Triangle);
    ...
    for (int i = 0; i < m_opt->tracker()->size(); i++) {
      x = y = 0;
      structure = m_opt->tracker()->at(i);
      QReadLocker structureLocker (structure->lock());
      // Don't plot removed structures or those who have not completed
      // their first optstep (energy < 1e-50).
      if ((structure->getStatus() != Structure::Optimized &&
           !showIncompletes)) {
        if  (!(structure->getStatus() == Structure::Duplicate &&
               showDuplicates)) {
          continue;
        }
      }

      if  (structure->getStatus() == Structure::Duplicate && !showDuplicates) {
        continue;
      }
      ...
      for (int j = 0; j < 2; j++) { // 0 = x, 1 = y
        ...
        case Structure_T:
          ...
          break;
        case Energy_T:
          // Skip Structures that don't have energy set
          if (structure->getEnergy() == 0.0) continue;
          switch (j) {
          case 0:       x = structure->getEnergy(); break;
          default:      y = structure->getEnergy(); break;
          }
          break;
        }
      }
      ...
      if (labelPoints) {
        ...
        case Index_L:
        default:
          pp->setLabel(QString::number(structure->getIndex()));
          break;
        case Energy_L:
          pp->setLabel(QString::number(structure->getEnergy(), 'g', 5));
          break;
        }
      }
    }
  ...
  void TabPlot::plotDistHist()
  {
    ...
    Structure* structure = m_opt->tracker()->at(ind);
    ...
    if (selected.size() == 0) {
      ...
      structure->getDefaultHistogram(&d, &f);
    ...
    // If atoms are selected:
      ...
      for (int i = 0; i < selected.size(); i++) {
        structure->generateIADHistogram(&d, &f_temp, 0, 15, .1,
