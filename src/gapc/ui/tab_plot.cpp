...
namespace GAPC {
    ...
    m_plotObject(0)
  {
    ...
    connect(ui.combo_plotType, SIGNAL(currentIndexChanged(int)),
            this, SLOT(refreshPlot()));
    connect(ui.combo_distHistPC, SIGNAL(currentIndexChanged(int)),
            this, SLOT(refreshPlot()));
    connect(ui.combo_distHistPC, SIGNAL(currentIndexChanged(int)),
  ...
  void TabPlot::readSettings(const QString &filename)
  {
    ...
    ui.combo_labelType->setCurrentIndex( settings->value("labelType", Structure_L).toInt());
  ...
  void TabPlot::plotTrends()
  {
    ...
    for (int i = 0; i < m_opt->tracker()->size(); i++) {
      ...
      for (int j = 0; j < 2; j++) { // 0 = x, 1 = y
        ...
        case Generation_T:
          switch (j) {
          case 0:       x = pc->getGeneration(); break;
          default:      y = pc->getGeneration(); break;
          }
          break;
        case Enthalpy_T:
  ...
  void TabPlot::plotDistHist()
  {
    ...
    ProtectedCluster *pc = qobject_cast<ProtectedCluster*>(m_opt->tracker()->at(ind));
