...
bool MtpFormat::read(Atoms::Geometry& s, const QString& filename)
{
  ...
  if (!Common::readFileToString(filename, &text)) {
    Common::error(QString("MTP file, %1, could not be opened!").arg(filename));
    return false;
  }
  ...
  if (begin == std::string::npos || end == std::string::npos) {
    Common::error("MTP file does not contain a complete CFG block.");
    return false;
  }
  ...
  while (getline(ifs, line)) {
    if (strstr(line.c_str(), "Size")) {
      ...
      if (lineSplit.size() != 1 || !Common::parseIntString(lineSplit[0], numAtoms)) {
        Common::error(QString("Could not read the number of atoms in MTP file! %1")
                     .arg(line.c_str()));
        return false;
      }
      ...
      for (size_t i = 0; i < 3; ++i) {
        ...
        if (lineSplit.size() != 3 || !Common::parseDoubleString(lineSplit[0], cellMatrix(i, 0)) ||
          ...
          Common::error(QString("Could not read the cell matrix in MTP file! %1")
                       .arg(line.c_str()));
          return false;
        }
      }
      ...
      for (int i = 0; i < numAtoms; i++) {
        ...
        if (lineSplit.size() < 5 || !Common::parseIntString(lineSplit[1], atomicType) ||
          ...
          Common::error(QString("Incomplete coords line in MTP file: %1")
                       .arg(line.c_str()));
          return false;
        }
      ...
      lineSplit = Common::split(line, ' ');
      if (lineSplit.size() < 3) {
        Common::error(QString("Could not read the chemical system in MTP file! %1")
                     .arg(line.c_str()));
        return false;
      }
  ...
  if (!cellFound)
    Common::error("Cell info was not found in MTP file!");
  if (!coordsFound)
    Common::error("Atom coords not found in MTP file!");
  if (!cellFound || !coordsFound)
    return false;
  if (atomicTypes.isEmpty()) {
    Common::error("No atomic types were found in MTP file!");
    return false;
  }
  ...
  if (min < 0 || max >= chemicalSystem.size()) {
    Common::error("Failed to read atomic types from MTP file!");
    return false;
  }
  ...
  for (int i = 0; i < atomicTypes.size(); i++) {
    ...
    if (atomicNum == 0) {
      Common::error(QString("Unrecognized element symbol in MTP file: %1")
                   .arg(sym));
      return false;
    }
